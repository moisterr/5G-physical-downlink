function Pi = get_3GPP_crc_interleaver_pattern(K)  
    % Define the maximum interleaver pattern defined by 3GPP TS 38.212 Table 5.3.1.2-2
    % This is a predefined permutation pattern used for interleaving CRC bits
    ...
    % Error if requested K is too large
    if K > length(Pi_IL_max)
        error('polar_3gpp_matlab:UnsupportedBlockLength','K should be no greater than 224.');
    end

    % Initialize the output interleaver pattern
    Pi = zeros(1,K);
    k = 0; 

    % Extract the last K indices from Pi_IL_max and adjust them
    for m = 0:length(Pi_IL_max)-1
        if Pi_IL_max(m+1) >= length(Pi_IL_max) - K
            % Offset the selected value to align with 1-based MATLAB indexing
            Pi(k+1) = Pi_IL_max(m+1) - (length(Pi_IL_max) - K) + 1; 
            k = k + 1;
        end
    end
end
